...
class App extends Component {
  ...
  handleAutoComplete(townAndState) {
    ...
    axios
      .get(
        `https://api.openbrewerydb.org/breweries?by_state=${capitalizedState}&page=2&per_page=25&by_type=micro`
      )
      .then(response => {
        ...
        //TODO:
        //this is where stateBreweryInfo should be filtered by distance
        //going to need to create an array of brewery towns
        
        console.log(stateBreweryInfo, "STATE BREWS");
        let breweryTowns = [];
        breweryTowns.push(
          stateBreweryInfo.map(brewery => {
        ...
        //might need to run each element of stateBreweryInfo through API and keep a record of distance
        //possibly create and object w/ name and distance
        ...
        let destinations = stateBreweryInfo.map(brewery => {
          return brewery.city + ', ' + brewery.state
        })
        ...
        service.getDistanceMatrix(
          ...
          function callback(response, status) {
            ...
            this.setState({ stateBeerInfo: stateBreweryInfo });
            console.log(this.state.stateBeerInfo, 'FINAL RESULT')
          }
      })
      .catch(error => {
        console.log(error);
